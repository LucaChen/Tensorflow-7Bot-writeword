package jp.narr.tensorflowmnist.model;
...
public class Arm7Bot {
    ...
    private int[] fluentRangeInit = {10, 10, 10, 10, 10, 10, 10};//流畅度
    private boolean[] reverse = {true, false, false, false, false, false, true};
    private double[] offsetInit = {-1, 0, -1, -8, -1, 66, 100};  // Unit: Degree
   ...
   public Arm7Bot(){
    ...
    public static int[] getReceivedMotor(int[] command){
        int[] motor=new int[7];
        int[] force=new int[7];
        int flag=command[16];
        int mul;
